...
import numpy as np
import utils
import utils.metrics_HKbig as Metrics_big
import utils.metrics_seq_HKbig as Metrics_seq_big
import utils.metrics_HKsmall as Metrics_small
import utils.metrics_seq_HKsmall as Metrics_seq_small
import random
...
class Encoder(tf.keras.Model):
    ...
    def __init__(self, N, h, w, embedding_dim, rate, timestep):
        ...
        self.rate = rate
        self.attn_layers = [AttnLayer(self.N, self.dim, self.w, self.rate) for _ in range(2)]
    ...
    def call(self, oddata, weather, sem_neighbor, geo_neighbor):
        ...
        data = tf.reshape(oddata, (-1, self.timestep, self.N, self.N))
...
class AttnSeq2Seq(tf.keras.layers.Layer):
    ...
    def __init__(self, N, h, w, embedding_dim, rate, timestep, out_seq_len, loss='MSE', is_seq=True, is_big=True):
