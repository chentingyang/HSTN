...
def mape(y_true, y_pred):
    return K.mean(tf.abs((y_true - y_pred) / y_true + epsilon)) * 100
...
def smape(y_true, y_pred):
    return 2.0 * K.mean(tf.abs(y_pred - y_true) / (tf.abs(y_pred) + tf.abs(y_true))) * 100
...
def d_mape(y_true, y_pred):
    ...
    return K.mean(tf.abs((y_true - y_pred) / y_true)) * 100
